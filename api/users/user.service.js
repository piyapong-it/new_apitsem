...
module.exports = {
  create: async (data, callBack) => {
    ...
    const queryResult = await pool
      ...
      .query(
        `insert into [USER] (userId, password, firstName, lastName, department, level, email, jdeCode)
  ...
  createUserByTSEM: async (data, callBack) => {
    const pool = await poolPromise;
    const queryResult = await pool
      .request()
  ...
  getUsers: async (callBack) => {
    ...
    const queryResult = await pool.request().query(
      `SELECT id, userId, password, firstName, lastName, department, level, email, jdeCode 
                    FROM [USER] 
                    ORDER BY userId`,
      (error, results, fields) => {
        if (error) {
  ...
  deleteUser: async (data, callBack) => {
    ...
    const queryResult = await pool
      .request()
      .input("Id", sql.Int, data.id)
      .query(`Delete [USER] where id = @Id `, (error, results, fields) => {
        if (error) {
          return callBack(error);
        }
        return callBack(null, results);
  ...
  getUserByUserId: async (userId, callBack) => {
    ...
    const queryResult = await pool
      ...
      .query(
        `SELECT id, userId, password, firstName, lastName, department, level, email, jdeCode, [APPVERSION] 
